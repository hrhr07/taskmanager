...
const data = [
  ...
  },
  {
    ...
    text: "Se le ha asignado una nueva tarea a usted y a otras dos personas. La prioridad de la tarea es alta, así que verifique y actúe en consecuencia. La fecha de la tarea es el viernes 7 de febrero de 2025. ¡¡¡Gracias!!!",
    task: {
  ...
  return (
    <>
      <Popover className="relative">
        <PopoverButton className="inline-flex items-center outline-none cursor-pointer">
                ...
                <div className="w-screen max-w-md flex-auto overflow-hidden rounded-3xl bg-white text-sm leading-6 shadow-lg ring-1 ring-gray-900/5">
                  <div className="p-4">
                    {data?.slice(0, 5).map((item, index) => (
                      <div
                        key={item._id + index}
                        className="group relative flex gap-x-4 rounded-lg p-4 hover:bg-[#2564ed2d]"
                      >
                        <div className="mt-1 h-8 w-8 flex items-center justify-center rounded-lg bg-[#2564ed2d] group-hover:bg-white">
                          {ICONS[item.notiType]}
                        </div>
                  ...
                  <div className="grid grid-cols-2 divide-x bg-gray-50">
                    {callsToAction.map((item) => (
                      <Link
                        ...
                        className="flex items-center justify-center gap-x-2.5 p-3 font-semibold text-blue-500 hover:bg-gray-100"
                      >
                        {item.name}
                      </Link>
                    ))}
